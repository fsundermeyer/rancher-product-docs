...
ifeval::["{build-type}" != "community"]
[#_whats_new_suse_rancher_prime_v2_14_1]
...
[#_rancher_identity_provisioning_with_scim]
...
[#_security_fixes_for_rancher_vulnerabilities]
== Security Fixes for Rancher Vulnerabilities

This release addresses the following Rancher security issues:

* Rancher now protects against arbitrary file access via path traversal in Rancher Extensions. Note by default only users with administrative permissions can deploy UI extensions unless explicit permission is granted to other users. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-5v3h-x4wf-5c35[CVE-2026-25705].
* Fleet's Helm deployer bypassed ServiceAccount impersonation in two places. A tenant with `git push` access to a Fleet-monitored repository could read secrets from any namespace on every downstream cluster targeted by their `GitRepo`. Both paths now use the impersonated credentials throughout and can no longer be bypassed. For more information, refer to https://github.com/rancher/fleet/security/advisories/GHSA-765j-qfrp-hm3j[CVE-2026-41050].
...
* A known issue exists where add-on charts' values are not preserved when viewing or editing clusters in the Rancher UI. Editing clusters via YAML is not affected. See https://github.com/rancher/dashboard/issues/17498[#17498].
...
*  A known issue occurs in air-gapped Harvester installations where the `ui-plugin-catalog` container `Compressed Endpoint` prevents installation. As a workaround, you can remove the `compressedEndpoint` via `kubectl edit` using the command `kubectl edit uiplugin harvester -n cattle-ui-plugin-system`. Once removed, the Harvester UI Plugin is immediately available. See https://github.com/rancher/rancher/issues/54396[#54396].
